...
def _client_keyboard():
    return types.ReplyKeyboardMarkup(
        keyboard=[[types.KeyboardButton(text=PROFILE_BUTTON)]],
        resize_keyboard=True,
    )
...
async def cmd_start(message: types.Message):
    ...
    if client and client.get("phone"):
        await message.answer(
            render_template("bot_after_phone", **{"КЛИНИКА": CLINIC_NAME}),
            reply_markup=_client_keyboard(),
        )
        return

    await message.answer(
        render_template("bot_welcome", **{"КЛИНИКА": CLINIC_NAME}),
...
async def got_contact(message: types.Message, state: FSMContext):
    ...
    # Уже зарегистрирован.
    after = render_template("bot_after_phone", **{"КЛИНИКА": CLINIC_NAME})
    db.save_message(client["id"], "out", after)
    await message.answer(after, reply_markup=_client_keyboard())
...
async def reg_last_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(render_template("bot_reg_firstname", **{"ФАМИЛИЯ": ln}))
...
async def cb_reminder_answer(call: types.CallbackQuery):
    ...
    confirm_text = render_template("bot_confirm")
    if answer == "yes":
...
SYSTEM_BUTTONS = {"📢 Рассылка", PROFILE_BUTTON, SKIP_BUTTON}
...
async def _scheduler_loop():
    """Фоновый цикл: раз в час проверяет напоминания и поздравления."""
